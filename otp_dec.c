 ...
 * Description: otp_dec.c file sets up socket with host server, sending and receiving data for OTP decryption. File structure based off of files provided by assignment instructions. This file is very similar/identical to otp_enc.c.
 */
...
int main(int argc, char *argv[])
{
	...
	if(strncmp(buffer, "error", 5) == 0){
		error("CLIENT: ERROR otp_dec cannot connect to otp_enc_d");
	}
	...
	// Iterate until all decrypted content from server has been received into buffer and written to file
	// Adapted from own code used in assignments from CS372 assignment on TCP file transfer, originally referenced from Linux/C documentation on man7.org
	while(bytesLeft > 0) {
